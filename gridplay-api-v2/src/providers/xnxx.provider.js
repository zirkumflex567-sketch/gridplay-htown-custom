...
const PROVIDER_ID = 'xnxx';
const DOMAINS = ['xnxx.com', 'www.xnxx.com', 'xnxx.es', 'www.xnxx.es', 'xnxx.tv'];
...
function classifyUrl(url) {
  ...
  if (pathname.includes('/playlist') || pathname.includes('/playlists')) {
    return { kind: 'playlist', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.9, signals: ['xnxx-playlist-path'] };
  }
  if (pathname.includes('/video') || pathname.includes('/watch')) {
    return { kind: 'video', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.9, signals: ['xnxx-video-path'] };
  }
  if (pathname.includes('/profile') || pathname.includes('/user') || pathname.includes('/channels')) {
    return { kind: 'profile', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.85, signals: ['xnxx-profile-path'] };
  }
  if (pathname.includes('/search') || search.includes('k=') || search.includes('search')) {
    return { kind: 'search', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.8, signals: ['xnxx-search-path'] };
  }

  return { kind: 'unknown', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.3, signals: ['xnxx-unknown-path'] };
}
...
function extractPlaylistItems(html) {
  ...
  for (const match of html.matchAll(pattern)) {
    const pageUrl = match[1].startsWith('http') ? match[1] : `https://www.xnxx.com${match[1]}`;
...
async function resolvePlaylist(url, ctx = {}) {
  ...
  return { pageUrl: url, items, total: items.length, resolverPath: ['provider', 'xnxx', 'html-playlist'] };
}
