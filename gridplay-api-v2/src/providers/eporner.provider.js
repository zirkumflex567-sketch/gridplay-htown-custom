...
const PROVIDER_ID = 'eporner';
const DOMAINS = ['eporner.com', 'www.eporner.com'];
...
function classifyUrl(url) {
  ...
  if (pathname.includes('/playlist') || pathname.includes('/playlists')) {
    return { kind: 'playlist', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.9, signals: ['eporner-playlist-path'] };
  }
  if (pathname.includes('/video') || pathname.includes('/watch')) {
    return { kind: 'video', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.9, signals: ['eporner-video-path'] };
  }
  if (pathname.includes('/profile') || pathname.includes('/model')) {
    return { kind: 'profile', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.85, signals: ['eporner-profile-path'] };
  }
  if (pathname.includes('/search') || search.includes('search')) {
    return { kind: 'search', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.8, signals: ['eporner-search-path'] };
  }

  return { kind: 'unknown', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.3, signals: ['eporner-unknown-path'] };
}
