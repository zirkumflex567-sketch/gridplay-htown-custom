...
const PROVIDER_ID = 'pornhub';
const DOMAINS = ['pornhub.com', 'www.pornhub.com', 'pornhub.org'];
...
function collectMediaCandidates(html) {
  ...
  const jsonLdPattern = /"contentUrl"\s*:\s*"([^"]+)"/gi;
  for (const match of normalized.matchAll(jsonLdPattern)) {
    const url = htmlDecode(match[1]);
    if (url.includes('.mp4') || url.includes('.m3u8')) {
      const type = url.includes('.mp4') ? 'mp4' : 'm3u8';
      if (!seen.has(url)) {
        seen.add(url);
        candidates.push({ url, mediaType: type, resolution: type === 'mp4' ? extractResolution(url) : 0 });
      }
    }
  }
...
function classifyUrl(url) {
  ...
  if (pathname.includes('/playlist') || pathname.includes('/playlists')) {
    return { kind: 'playlist', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.9, signals: ['pornhub-playlist-path'] };
  }
  if (pathname.includes('/video') || pathname.includes('/view')) {
    return { kind: 'video', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.9, signals: ['pornhub-video-path'] };
  }
  if (pathname.includes('/profile') || pathname.includes('/users') || pathname.includes('/model') || pathname.includes('/pornstar')) {
    return { kind: 'profile', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.85, signals: ['pornhub-profile-path'] };
  }
  if (pathname.includes('/search') || search.includes('search') || search.includes('q=')) {
    return { kind: 'search', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.8, signals: ['pornhub-search-path'] };
  }

  return { kind: 'unknown', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.3, signals: ['pornhub-unknown-path'] };
}
...
async function defaultFetch(url, options = {}) {
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'Referer': `${new URL(url).protocol}//${new URL(url).hostname}/`
    },
...
function extractDuration(html) {
  const match = html.match(/"duration"\s*:\s*"?(\d+)"?/i) || html.match(/(\d+):(\d+)/);
  if (match && match[2]) return parseInt(match[1]) * 60 + parseInt(match[2]);
...
function extractPlaylistItems(html) {
  const items = [];
  const seen = new Set();
  const pattern = /href="(\/video[^\"]+)"[^>]*title="([^"]+)"/gi;
  for (const match of html.matchAll(pattern)) {
    const pageUrl = match[1].startsWith('http') ? match[1] : `https://www.pornhub.com${match[1]}`;
...
async function resolvePlaylist(url, ctx = {}) {
  ...
  return { pageUrl: url, items, total: items.length, resolverPath: ['provider', 'pornhub', 'html-playlist'] };
}

module.exports = { id: PROVIDER_ID, domains: DOMAINS, classifyUrl, resolveVideo, resolvePlaylist };