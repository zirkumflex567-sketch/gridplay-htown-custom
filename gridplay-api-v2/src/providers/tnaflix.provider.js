...
const PROVIDER_ID = 'tnaflix';
const DOMAINS = ['tnaflix.com', 'www.tnaflix.com'];
...
function classifyUrl(url) {
  ...
  if (pathname.includes('/playlist') || pathname.includes('/playlists')) {
    return { kind: 'playlist', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.9, signals: ['tnaflix-playlist-path'] };
  }
  if (pathname.includes('/video') || pathname.includes('/watch')) {
    return { kind: 'video', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.9, signals: ['tnaflix-video-path'] };
  }
  if (pathname.includes('/profile') || pathname.includes('/model') || pathname.includes('/pornstar')) {
    return { kind: 'profile', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.85, signals: ['tnaflix-profile-path'] };
  }
  if (pathname.includes('/search') || search.includes('search')) {
    return { kind: 'search', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.8, signals: ['tnaflix-search-path'] };
  }

  return { kind: 'unknown', providerId: PROVIDER_ID, canonicalUrl: url, confidence: 0.3, signals: ['tnaflix-unknown-path'] };
}
...
function extractDuration(html) {
  ...
  // 2. Fallback to generic timestamp MM:SS
